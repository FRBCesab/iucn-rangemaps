...
#' @examples
#' \dontrun{
#' birds <- birds_read_data()
#' birds <- birds_rename_subpop(birds)
#' birds <- birds_rename_species(birds)
#' sp_list <- birds_get_species_info(birds)
#' sp_list <- birds_match_taxonomies(sp_list)
#' }

birds_match_taxonomies <- function(data) {
  ...
  cols <- c("blife_id", "blife_binomial", "blife_order", "blife_family", 
            "blife_iucn_status")
  ...
  if (!file.exists(file.path(path, "sp_list_aves_final.csv"))) {
    stop("The file 'sp_list_aves_final.csv' does not exist in '", path, 
         "/'.", call. = FALSE)
  }
  
  ref <- read.csv2(file.path(path, "sp_list_aves_final.csv"))
  ref <- ref[ , c("ref_id", "ref_binomial", "ref_order", "ref_family")]
  
  
  ## Match by IUCN ID ----
  
  taxo_1 <- merge(data, ref, by.x = "blife_id", by.y = "ref_id", all = FALSE)
  
  taxo_1$"ref_id" <- taxo_1$"blife_id"
  taxo_1$"method" <- "Match by ID"
  
  
  ## Match by binomial ----
  
  spp <- data[which(!(data$"blife_id" %in% taxo_1$"blife_id")), ]
  
  taxo_2 <- merge(spp, ref, by.x = "blife_binomial", by.y = "ref_binomial", 
                  all.x = TRUE, all.y = FALSE)
  
  taxo_2$"ref_binomial" <- taxo_2$"blife_binomial"
